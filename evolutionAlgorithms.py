...
class ClimbingHillAlgorithm(AbstractAlgorithm):
    ...
    def step(self):
        ...
        for i in range(len(self.population)):
            ...
            N = neighborhood(x[:2], 1, fitness_fn=self.fitnessFunction)  # or x[:(len(x)-1)]
            best = N[0]
...
def neighborhood(x, d, fitness_fn=None, n=10):
    """
...
class SimulatedAnnealingAlgorithm(AbstractAlgorithm):
    ...
    def step(self):
        ...
        for i in range(len(self.population)):
            ...
            for j in range(10):
                # we assume that last parameter is fitness
                N = neighborhood(x[:2], 1, fitness_fn=self.fitnessFunction, n=1)  # or x[:(len(x)-1)]
